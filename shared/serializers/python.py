...
from shared.serializers import base
...
class Serializer(base.Serializer):
    ...
    def handle_field(self, obj, field):
        ...
        if self.excluded_fields is not None and field.attname in self.excluded_fields:
            return
    ...
    def pk_handle(self,obj,field):
        fielddic = {}
        related = getattr(obj, field.name)
        from django.db.models.fields.files import ImageField
        for f in related._meta.fields:
            if self.excluded_fields is not None and f.attname in self.excluded_fields:
                continue
            if f.remote_field is not None and f.primary_key:
                continue
            if f.remote_field is None:
                if isinstance(f, ImageField):
                    fielddic[f.name] = f.value_from_object(related).url
                else:
                    fielddic[f.name] = f.value_from_object(related)
            else:  # pk
                fielddic[f.name] = self.pk_handle(related, f)
        return fielddic
    ...
    def handle_fk_field(self, obj, field):
        ...
        if self.use_recursion:
            self._current[field.name] = self.pk_handle(obj, field)
        else:
            fielddic = {}
            related = getattr(obj, field.name)
            if related is not  None:
                for f in related._meta.fields:
                    ...
                    if isinstance(f, ImageField):
                        ...
                        fielddic[f.name] = f.value_from_object(related)
            self._current[field.name] = fielddic
            ...
            def m2m_value(value):
                ...
                for field in value._meta.fields:
                    ...
                    if isinstance(field, ImageField):
                        fielddic[field.name] = field.value_from_object(related).url
                    else:
                        fielddic[field.name] = field.value_from_object(value)
                return fielddic
